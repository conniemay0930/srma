...
def format_abstract(abstract: str) -> str:
    ...
    # ADDED RESULT and DISCUSSION to the regex group
    abstract = re.sub(
...
def parse_pubmed_result(xml_string: str) -> List[Dict[str, str]]:
    ...
    # 命名空間（防止命名空間影響尋找）
    # 由於 PubMed EFetch 的 XML 通常沒有命名空間，直接使用標籤名稱
...
def get_model(api_key: str, model_name: str):
    ...
    if not api_key:
        st.error("請在設定頁籤中輸入 Google AI API Key。")
        return None
        
    try:
        client = genai.Client(api_key=api_key)
        # Try a quick call to ensure the key is valid and model is accessible
        client.models.get(model=model_name)
        return client
    except APIError as e:
        st.error(f"API Key 或模型連線錯誤: {e.message}")
        return None
    except Exception as e:
        st.error(f"初始化模型錯誤: {e}")
        return None
...
def get_locale():
    ...
    return {
        ...
        "tabs_step34": "Step 3+4 Records + 粗篩", # <--- MODIFIED LINE
...
# Custom Sidebar
# =========================================================
# ... (The sidebar code remains unchanged in this file, so it's omitted for brevity but included in the full script)

# =========================================================
# Main Tabs Layout
# =========================================================
# ... (The tabs setup code remains unchanged)

# ... (The content for Tab 0, Tab 1, and Tab 2 remains unchanged)

# =========================================================
# Tab 3: Step 3+4 Records + 粗篩
# =========================================================
with tabs[3]:
    ...
    if not studies_df.empty:
        ...
        # START OF MODIFIED LOGIC: Group studies by final_lbl
        grouped_studies = {
            "INCLUDE": [],
            "EXCLUDE": [],
            "UNSURE": []
        ...
        for group_key in order:
            ...
            if studies_in_group:
                ...
                for pmcid, study in studies_in_group:
                    ...
                    # Determine Expander color/icon based on final_lbl
                    expander_color = "blue"
                    expander_icon = ""
...
# =========================================================
# ... (The rest of the code for Tab 5, 6a, 6b, 7, 8, and 9 remains unchanged, so it's omitted for brevity but included in the full script)
# =========================================================
