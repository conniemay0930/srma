...
with colA:
    include_CO = st.checkbox("PubMed 搜尋式納入 C/O（通常會降低召回；預設不勾）", value=False)
with colB:
    add_rct = st.checkbox("自動加入 RCT 關鍵字", value=(st.session_state.get("study_type","").startswith("RCT")))
...
# ---- Title/abstract screening + full text review (merged) ----
st.markdown("---")
...
# compute AI suggestions if not cached
...
for _, row in df_records.iterrows():
    ...
    if llm_available():
        try:
            ...
            st.session_state["ta_ai"][rid] = {"decision": "Unsure", "confidence": 0.45, "reason": f"LLM 失敗，改規則法：{e}"}
    else:
        st.session_state["ta_ai"][rid] = ai_screen_rule_based(row, pico, study_filter)

def compute_effective_ta(rid: str) -> str:
    ov = st.session_state["ta_override"].get(rid, "")
    if ov in ["Include", "Exclude", "Unsure"]:
        return ov
    return st.session_state["ta_ai"].get(rid, {}).get("decision", "Unsure")

# KPIs
ta_decisions = [compute_effective_ta(r) for r in df_records["record_id"].tolist()]
ta_inc = sum(1 for d in ta_decisions if d == "Include")
ta_exc = sum(1 for d in ta_decisions if d == "Exclude")
ta_uns = sum(1 for d in ta_decisions if d == "Unsure")

st.markdown(
    f"<div class='kpiwrap'>"
    f"<div class='kpi'><div class='label'>Title/Abstract：Include</div><div class='value'>{ta_inc}</div></div>"
    f"<div class='kpi'><div class='label'>Title/Abstract：Exclude</div><div class='value'>{ta_exc}</div></div>"
    f"<div class='kpi'><div class='label'>Title/Abstract：Unsure</div><div class='value'>{ta_uns}</div></div>"
    f"</div>",
    unsafe_allow_html=True
)

# unified display
st.markdown("<div class='notice'><b>提示</b>：展開每篇可看摘要（分段），可保留 AI 解釋並做人工修正；Full text review 亦在同一篇底下。</div>", unsafe_allow_html=True)

for _, r in df_records.iterrows():
    rid = r["record_id"]
    title = r.get("title","")
    ai = st.session_state["ta_ai"].get(rid, {})
    ...
    meta_line = f"PMID: {r.get('pmid','')}    DOI: {r.get('doi','') or '—'}    Year: {r.get('year','') or '—'}    First author: {r.get('first_author','') or '—'}    Journal: {r.get('journal','') or '—'}"
    with st.expander(f"{rid}｜{short(title, 110)}", expanded=False):
        st.markdown(f"<span class='badge {badge}'>Effective：{eff}</span>"
                    f"<span class='badge'>AI：{ai_dec}</span>"
                    f"<span class='badge'>信心度：{ai_conf:.2f}</span>", unsafe_allow_html=True)
        ...
        if abs_txt.strip():
            for para in abs_txt.split("\n\n"):
                st.write(para.strip())
        else:
            st.write("（無摘要）")

        st.markdown(f"**AI Title/Abstract 建議**：{ai_dec}（信心度 {ai_conf:.2f}）")
        st.markdown(f"**理由**：{ai_reason or '（無）'}")
        ...
        if st.button("儲存粗篩修正", key=f"save_ta_{rid}"):
            st.session_state["ta_override"][rid] = new_dec
            st.session_state["ta_override_reason"][rid] = new_reason
            st.success("已儲存。")

        st.markdown("---")
        st.markdown("**Full text review**（可上傳 PDF 或貼全文；全文排除需附理由）")
        ft_dec_default = st.session_state["ft_decisions"].get(rid, "Not reviewed yet")
        ft_dec = st.selectbox(
            "Full text 決策",
            options=["Not reviewed yet", "Include for meta-analysis", "Exclude after full-text"],
            index=["Not reviewed yet", "Include for meta-analysis", "Exclude after full-text"].index(ft_dec_default if ft_dec_default in ["Not reviewed yet","Include for meta-analysis","Exclude after full-text"] else "Not reviewed yet"),
            key=f"ft_dec_{rid}"
        )
        ft_reason = st.text_area(
            "Full text 排除理由/備註（若排除請必填）",
        ...
        ft_text = st.text_area(
            ...
            value=st.session_state["ft_text"].get(rid, ""),
            height=160,
            key=f"ft_text_{rid}"
        )
        if st.button("儲存 Full text review", key=f"save_ft_{rid}"):
            st.session_state["ft_decisions"][rid] = ft_dec
...
# candidate set: FT include for MA; if none yet, fall back to TA include
ft_includes = [rid for rid, d in st.session_state["ft_decisions"].items() if d == "Include for meta-analysis"]
if not ft_includes:
    ft_includes = [rid for rid in df_records["record_id"].tolist() if compute_effective_ta(rid) == "Include"]

cands = df_records[df_records["record_id"].isin(ft_includes)].copy()
if cands.empty:
    st.warning("目前沒有可供 extraction 的文章（建議先在粗篩/全文決策至少選 Include）。")
else:
    base = cands[["record_id","pmid","year","doi","journal","first_author","title"]].copy()
    ensure_columns(base, ["Outcome_label","Timepoint","Effect_measure","Effect","Lower_CI","Upper_CI","Effect_unit","Notes"], default="")
    ...
    # add schema outcome columns (optional extra fields)
    for oc in schema:
        coln = f"Outcome:{oc}"
        if coln not in base.columns:
            base[coln] = ""

    st.caption("缺資料/不合法會在下方以紅字提醒；不會卡住流程。若 OR/RR/HR 的 effect/CI 有 0 或負值，將自動略過該筆進入 MA。")

    # use form to reduce rerun jitter
    ...
    # normalize to DataFrame & ensure columns (FIX KeyError)
    ...
    ensure_columns(ex, ["Effect","Lower_CI","Upper_CI","Effect_measure","Outcome_label","Timepoint"], default="")
    st.session_state["extract_df"] = ex
    if saved:
        st.session_state["extract_last_saved"] = time.time()
        st.success("已儲存寬表。下一步請到 Step 6 按『更新森林圖/MA』。")

    # Validation summary (non-blocking)
    v = ex.copy()
    ...
    for _, row in v.iterrows():
        ...
        if pd.notna(row.get("Lower_CI")) and pd.notna(row.get("Upper_CI")):
            try:
                if float(row.get("Upper_CI")) <= float(row.get("Lower_CI")):
                    invalid.append("Upper_CI <= Lower_CI")
            except Exception:
                pass

        if missing or invalid:
            issues.append({"record_id": rid, "title": title, "missing": ", ".join(missing), "invalid": "; ".join(invalid)})

    if issues:
        st.markdown("<div class='card'><b>資料檢核（不會卡住，但建議修正）</b><br>"
                    "<span class='red'>紅色提示代表缺資料或數值不合法；不修正仍可下一步，但該筆可能不會納入 MA。</span></div>",
                    unsafe_allow_html=True)
        st.dataframe(pd.DataFrame(issues), use_container_width=True, height=240)
    else:
    ...
    if llm_available():
        try:
            msg = [
                ...
                {"role":"user", "content": f"""
請用繁體中文撰寫一份 SR/MA 稿件草稿（分段），語氣可參考 style guide（若有）。
要求：
- 必須分成：標題、摘要、前言、方法、結果、討論、結論（可另加限制/未來研究）。
- 若缺資料或需人工補填，請用全形『』標記空白位置，不要硬編。
- 可引用：PICO、criteria、PubMed 搜尋式、PRISMA 計數、MA 結果（若有）。
- ROB 2.0、全文排除理由等若未完成，也要留段落與『』提示。

PICO: {json.dumps(pico, ensure_ascii=False)}
Criteria: {st.session_state.get("criteria_text","")}
PubMed query: {st.session_state.get("pubmed_query","")}
PRISMA counts: {json.dumps(counts, ensure_ascii=False)}
MA result: {json.dumps(ma_result, ensure_ascii=False)}

Style guide (optional; may be empty):
{style_text[:12000]}
"""}
            ]
            ...
            st.session_state["ms_sections"] = manuscript_template(proto, ma_result, lang_code)
    else:
        st.session_state["ms_sections"] = manuscript_template(proto, ma_result, lang_code)
